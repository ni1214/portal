...
## Stitch デザイン運用ルール（最優先）

> **目的**: UI デザインは今後 `Stitch` を前提に進める。画面要件は Portal 側で定義し、見た目・情報配置・ライト/ダーク/モバイルの展開は Stitch に任せる。

### 基本ルール
- 新しい画面や大きな UI 改修では、まず **Stitch にデザイン案を考えさせる前提**で進める
- **ホーム画面、各モーダル、主要パネル**はすべて `PC / スマホ / ダーク / ライト` をセットで考える
- ユーザーが細かい見た目を指定していない場合、**機能要件だけ守ってビジュアル判断は Stitch 側に委ねる**
- 特に **ホーム以外のモーダル** は、現行機能を満たすことを優先しつつ、細部のレイアウト・強弱・余白・装飾は Stitch 主導でよい
- ユーザーから **Stitch の project/share URL** を渡されたら、それを最優先の参照元とする
- URL が未共有・404・認証不足で読めない場合は、**Stitch 方針（大胆な情報整理、PC/スマホ別最適化、ダーク/ライト両立）を維持したまま実装を先に進める**

### 実装ルール
- Stitch 案を反映するときも、**既存の機能 ID・イベント導線・データ構造は壊さない**
- UI を刷新しても、`保存 → 閉じる → 再表示 → 関連画面へ反映` まで既存機能が成立すること
- デザイン都合で DOM を変える場合も、JS のイベントターゲットになる `id` や `data-*` は互換を保つ
- 画面全体の雰囲気は自由度を上げてよいが、**可読性・操作性・テーマ切替・モバイル操作**は必須要件
- ホームは `ヒーロー領域 / 状況メトリクス / 優先導線 / 詳細カード` のように、**優先順位が一目で分かる情報設計**を優先する
- モーダルは機能を満たしていれば、**レイアウト・強調・余白・カード構成・ナビゲーション表現は Stitch 側の裁量を広く認める**

### テーマ運用
- Stitch に依頼するときは **ダークモードだけでなくライトモードも同時に成立する案** を前提にする
- スマホ版は「縮小した PC 版」ではなく、**情報優先順位を整理した専用レイアウト**として扱う
- 色はハードコードを増やさず、既存 CSS 変数か、新設する場合も `:root / [data-theme="light"]` の両方に定義する
