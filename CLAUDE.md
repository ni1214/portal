...
### 🗑️ ルール9: 完了タスクは必ず CLAUDE.md から削除する

- `## 🔧 積み残しタスク` / `## 次回実装予定タスク` に記載したタスクが完了したら、**その場で該当項目を削除**してコミットする
- 完了済みタスクを残すと「やったかどうか分からない」状態になり、次の担当 Claude が二重作業や誤判断をする原因になる
- タスクが「おそらく完了している」と思われる場合も、実際にコードを確認して確実に完了していれば削除する

### 🔔 ルール10: ブラウザネイティブ通知を使わない（notify.js を必ず使う）
