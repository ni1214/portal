...
## 🧭 初見ユーザー視点レビュールール（UX再発防止・最重要）

> **背景**: 2026-03、クイックアクセスパネルとサイドバーが100%同じ内容を表示し「どっちを使えばいいか」という認知コストが発生した。
> 実装前・実装後に必ず以下のチェックを行うこと。

### ✅ UIを追加・変更するときの「初見ユーザー10秒チェック」

このポータルを**初めて見る人間がホーム画面を10秒見た**ときに以下の疑問が生まれないか？

| チェック項目 | NGの例 | 修正方針 |
|---|---|---|
| **同じ操作が2か所にある** | サイドバーとクイックアクセスに同じボタン | どちらか一方を削除、または役割を明確に分ける |
| **ラベルが違うのに同じ機能** | 「勤怠入力」と「カレンダー・勤怠」が別ボタン | 名称を統一、または統合する |
| **なぜここにあるのか説明できない** | ホーム画面にあるのにサイドバーでも行ける | 存在意義を説明できないUIは削除 |
| **操作の起点が複数ある** | FAB・サイドバー・クイックアクセスの3か所から同じモーダルが開く | 1か所に統一 |
| **空のパネルが並んでいる** | ロード前に空のグレーボックスが複数並ぶ | スケルトンUI or ローディング表示を追加 |

### 🔑 ナビゲーション設計原則

```
サイドバー（デスクトップ常時表示）= プライマリナビ → ここに全機能
ボトムナビ（モバイル常時表示）   = モバイルプライマリナビ → 主要5機能のみ
クイックアクセス（ホーム内）    = モバイル補助のみ / デスクトップでは表示しない
ホーム画面のコンテンツ          = ナビゲーションではなく「状態・数値・お知らせ」を表示
```

**禁止パターン**:
- ❌ 同じ機能へのリンクが3か所以上ある
- ❌ 「クイックアクセス」にサイドバーと同じ項目を並べる（デスクトップ）
- ❌ ホーム画面をナビゲーションボードにする（Googleのホームは検索のみ）

### 🤔 「なぜここにあるのか」を説明できるか判定フロー

```
新しいUI要素を追加する前に:
  1. この要素は「情報（数値・状態・お知らせ）」か「ナビゲーション（リンク・ボタン）」か？
  2. ナビゲーションなら → サイドバー/ボトムナビに入れる。ホームには置かない。
...
> **エージェント追加ルール**: ユーザーが「〇〇エージェントを追加して」と言ったら、即座にこの CLAUDE.md の常設エージェント一覧に追記すること。エージェントの数に制限はなく、役割が明確であれば何個でも追加してよい。

### 常設エージェント一覧

| エージェント | 役割 | 主な作業内容 |
|---|---|---|
| **🎯 指揮官 (Commander)** | 全体統括 | ユーザーの発言を解釈して必要なエージェントを選定・起動する。タスク分割・エージェント割り当て・進行管理。不明な場合は「〇〇エージェントと〇〇エージェントを使います」と宣言してから作業開始 |
...
| **📱 レスポンシブチェッカー (Responsive Checker)** | 表示幅別レイアウト確認 | `preview_resize` で mobile/tablet/desktop 3サイズのスクリーンショットを撮り崩れを検出・報告 |

### 📱 レスポンシブチェッカー 手順（新規UI追加・修正後に必ず実行）

レスポンシブチェッカーは以下の3サイズで `preview_resize` + `preview_screenshot` を実行し、崩れを検出する。

| サイズ | 幅 × 高さ | 確認ポイント |
|---|---|---|
| **mobile** | 375 × 812 | ボトムナビ表示・カード1列・テキスト折り返しなし |
| **tablet** | 768 × 1024 | 2カラム or 1カラム切り替わり点・モーダル幅 |
| **desktop** | 1280 × 900 | サイドバー常時表示・2カラムレイアウト・クイックアクセス非表示 |

**NGパターン（発見したらバグファインダー→バグフィクサーへエスカレート）**:
- テキスト・バッジが縦に折り返している
- 要素がはみ出してスクロールバーが横に出る
- コンポーネントが重なっている・見切れている
- グリッドが1列に崩れてコンテンツが縦長になりすぎる
- ボタン・アイコンのタップ領域が小さすぎる（44px未満）

**実行コマンド例**:
```
preview_resize({ width: 375, height: 812 })  → preview_screenshot()
preview_resize({ width: 768, height: 1024 }) → preview_screenshot()
preview_resize({ width: 1280, height: 900 }) → preview_screenshot()
```
...
| **mobile/tablet/desktopの3サイズで崩れていないか** | **レスポンシブチェッカーに依頼（375/768/1280px）** |
